...
thingstowrite.append("	(:domain Kart)")
...
def makeisinbetweenstring(i,j,k,l):
	#Above lies
	return "( abovelies Grid_"+str(i)+"_"+str(j) + " Grid_" + str(k)+"_"+str(l) + " )\n"

def makeisinbetweenstring2(i,j,k,l):
	#Left lies
	return "( leftlies Grid_"+str(i)+"_"+str(j) + " Grid_" + str(k)+"_"+str(l) + " )\n"
...
for i in xrange(1,1+dimy):
	for j in xrange(1,1+dimx):
		if boolgrids[i][j]==0:
			...
			carstrings.append("( taken Car_"+boolgrids[i][j]+" Grid_"+str(i)+"_"+str(j)+" )\n") 
